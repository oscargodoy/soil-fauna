...
res.bv.step.biobio2  <- bv.step(sp, sp,  
                                ...
                                num.restarts=50, 
                                output.best=10, 
                                var.always.include=c(3,4,5,9,10)) 
res.bv.step.biobio2 
...
env.fit <- envfit(MDS_res, env[,c(1,5,16)], perm = 999) 
env.fit 
...
#Do the same with the environmental variables, 
env.fit <- envfit(MDS_res, env2[,c(7,15,16)], perm = 999)  
env.fit 
